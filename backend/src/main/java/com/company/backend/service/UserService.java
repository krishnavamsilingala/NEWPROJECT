...
import com.company.backend.persistence.dao.PasswordResetTokenRepository;
import com.company.backend.persistence.dao.RoleRepository;
import com.company.backend.persistence.dao.UserRepository;
import com.company.backend.persistence.dao.VerificationTokenRepository;
...
import com.company.backend.web.error.UserAlreadyExistAuthenticationException;
...
public class UserService implements IUserService {
    ...
    public User registerNewUserAccount(final UserDto accountDto) {
		...
		user.setPassword(passwordEncoder.encode(accountDto.getMatchingPassword()));
		user.setEnabled(false);
		user.setSocialid(null);
		user.setProvider("LOCALUSER");
        user.setRoles(Arrays.asList(roleRepository.findByName("ROLE_USER")));
        return repository.save(user);
    }
    
    @Override
    public User registerNewSocialAccount(User user) {
    	 User userExist=repository.findBySocialid(user.getSocialid());
	       
    	 if (userExist != null) {
	        	throw new UserAlreadyExistAuthenticationException("User with email id " + user.getEmail()+ "already exist");
	        }
    	return repository.save(user);
    }
